...
public class Windows {
    ...
    public static void main(String[] args) throws Exception {
        List<WindowInfo> inflList = listAllWindows();
        List<WindowInfo> filteredWindows = inflList.stream().filter(d -> d.title.startsWith("Knight")).collect(Collectors.toList());
        for (WindowInfo w : filteredWindows) {
            System.out.println(w);
        }

        int hWnd = User32.instance.FindWindowA(null, "Knight OnLine Client");
        ...
        ImageIO.write(createScreenCapture, "png", new File("D:\\Tess4J\\tessdata\\partySSS.png"));
    }

    private static List<WindowInfo> listAllWindows() throws AWTException, IOException {
